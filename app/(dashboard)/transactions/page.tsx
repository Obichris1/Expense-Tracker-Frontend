...
const inputClass =
  "w-full rounded-lg border border-gray-200 px-4 py-2.5 text-sm text-gray-800 outline-none focus:border-gray-400 placeholder:text-gray-400";
const selectClass =
  "w-full appearance-none rounded-lg border border-gray-200 px-4 py-2.5 text-sm text-gray-800 outline-none focus:border-gray-400 bg-white";
...
export default function TransactionsActivity() {
  ...
  >({
    ...
    queryFn: () =>
      getTransactions({ s: searchQuery || undefined, page, limit }),
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      <PaymentDialog
        ...
        onSubmit={(accountId, data) =>
          addTransactionMutation.mutate({ accountId, data })
        }
...
function PaymentDialog({
  ...
  const [scannedReceipt, setScannedReceipt] = useState<ReceiptData | null>(
    null
  );
    ...
    try {
      ...
      setFormData((prev) => ({
        ...
        description: data.merchant
          ? `${data.merchant}${
              data.notes ? ` — ${data.notes.slice(0, 60)}` : ""
            }`
          : prev.description,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const selectedAccount = accounts.find(
      (acc) => acc.id === Number(formData.accountId)
    );
  ...
  return (
    ...
    >
      <DialogTitle
        sx={{ padding: "20px 24px 16px", borderBottom: "1px solid #f0f0f0" }}
      >
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            ...
            style={{
              background:
                "linear-gradient(to right, #f97316, #ec4899, #a855f7)",
            }}
          >
            {isScanning ? (
              <>
            ...
            <div className="flex items-start gap-3 rounded-lg border border-green-200 bg-green-50 px-3 py-2.5">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-green-800">
                  {scannedReceipt.merchant} — {scannedReceipt.currency}
                  {scannedReceipt.totals?.total?.toFixed(2)}
                </p>
                <p className="text-xs text-green-700 mt-0.5 truncate">
                  {scannedReceipt.line_items?.length} items ·{" "}
                  {scannedReceipt.category}
